...
class DFTEmbeddingSolver:
    ...
    def solve(self, driver: PySCFDriver,mol_name: str = "Molecule", active_space_str: str = "CAS"):
        ...
        while n_iter < self.max_iter:
            ...
            # ==========================================================
            # 🔹 SPIN PENALTY HAMILTONIAN BUILD (ANION / DOUBLET FIX)
            # ==========================================================
            ...
            # 3. Build the Penalized Hamiltonian for a Doublet
            # Benzene: 0.33828
            # Naphthalene: 0.21432
            # Anthracene : 0.14648
            # Tetracene  : 0.10110
            ...
            # Perylene : 0.12863
            beta = 0.10110  # <--- Replace with your calculated beta value for this molecule

            # Since S=1/2, the true S^2 eigenvalue is 0.75.
            # We subtract 0.75 * beta using an Identity operator ("") so the
            # penalty is exactly 0.0 for the true anion ground state.
            num_spin_orbs = as_problem.num_spatial_orbitals * 2
            identity_shift = FermionicOp({"": -0.75 * beta}, num_spin_orbitals=num_spin_orbs)

            penalized_h_op = h_op + (beta * s2_op) + identity_shift
...
def _main():
    ...
    # Assuming your neutral system was 2e, 6o. The cation becomes 1e, 6o.
...
# ---------- 🔹 VQE callback with quantum timing & convergence tracking ----------
    ...
    # ================================================================
    try:
        mol_name = "Tetracene_mu"  # Change this based on the geometry used (e.g., "H2O", "CO2", "Benzene", etc.)
