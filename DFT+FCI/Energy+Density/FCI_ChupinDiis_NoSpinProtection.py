...
class DFTEmbeddingSolver:
    ...
    @staticmethod
    def damp_active_density(density_history, base_alpha=0.75, diis_threshold=1e-3, delta=1e-4):
        """
        Streamlined Adaptive DIIS damping (Chupin 2021 Algorithms 3 & 4).
        """
        ...
        prev_mat, new_mat = get_mat(density_history[-2]), get_mat(density_history[-1])
        curr_res_norm = np.linalg.norm(new_mat - prev_mat, 'fro')
        print(f"[Convergence Check] Residual Norm = {curr_res_norm:.3e}")

        # 2. Adaptive DIIS Phase (Chupin Threshold & Depth)
        if curr_res_norm < diis_threshold and len(density_history) >= 3:
            try:
                # Calculate historical residual norms efficiently
                mats = [get_mat(d) for d in density_history]
                errors = [mats[i] - mats[i-1] for i in range(1, len(mats))]
                norms = [np.linalg.norm(e, 'fro') for e in errors[:-1]]

                # Adaptive depth: count valid historical steps where delta * |r(i)| < |r(k+1)|
                valid_count = sum(1 for norm in norms if delta * norm < curr_res_norm)
                n = max(2, min(valid_count + 1, 6)) # Size of DIIS subspace (min 2, max 6)

                # Slice the active subspace errors and target densities
                sub_errors = errors[-n:]
                sub_densities = density_history[-n:]

                # Build DIIS B-matrix compactly
                B = np.zeros((n + 1, n + 1))
                # Compute inner products for the upper NxN block using C-level einsum
                inner_prods = np.einsum('aij,bij->ab',
                                        np.array(sub_errors),
                                        np.array(sub_errors))
                B[:n, :n] = inner_prods + np.eye(n) * 1e-8 # with regularization
                ...
                # Solve and extrapolate
                coeffs = np.linalg.solve(B, rhs)[:n]
                print(f"[Adaptive DIIS] Space m={n}, Coeffs: {np.round(coeffs, 2)}")

                # Reconstruct density object safely for Qiskit types
                diis_density = None
                for c, d in zip(coeffs, sub_densities):
                ...
                print(f"[Adaptive DIIS Warning] Matrix solve failed ({e}); falling back to linear damping.")

        # 3. Linear Damping Phase (Burn-in)
        alpha = max(0.05, base_alpha / np.sqrt(len(density_history)))
        print(f"[Damping] Linear mix with alpha={alpha:.3f}")
        try:
            return (1.0 - alpha) * density_history[-2] + alpha * density_history[-1]
        except Exception:
            return density_history[-1]
...
def _main():
    ...
    driver = PySCFDriver(
        atom=tetracene,  # Change this to the geometry you want to use (e.g., geometry_C6H6, geometry_C10H8, etc.)
        basis="6-31g*",
    ...
    # ================================================================
    try:
        mol_name = "FCI_Tetracene_lrc_wpbe"  # Change this based on the geometry used (e.g., "H2O", "CO2", "Benzene", etc.)
