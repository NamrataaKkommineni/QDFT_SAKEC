...
def _main():
    ...
    # 1. Define the dynamic filter INSIDE _main() so it can see active_num_electrons
    def enforce_particle_and_spin(eigenstate, eigenvalue, aux_values):
        if aux_values is None:
            return True

        for key, val_tuple in aux_values.items():
            key_str = str(key)
            if "ParticleNumber" in key_str:
                # Dynamically checks against whatever you set at the top of the script
                if not np.isclose(val_tuple[0], float(active_num_electrons)):
                    return False
            if "Magnetization" in key_str:
                # Forces a Singlet state (S_z = 0)
                if not np.isclose(val_tuple[0], 0.0):
                    return False

        return True

    # 2. Apply the dynamic filter to the exact solver
    from qiskit_algorithms import NumPyMinimumEigensolver
    exact_solver = NumPyMinimumEigensolver(filter_criterion=enforce_particle_and_spin)
    ...
    # ================================================================
    try:
        mol_name = "FCI_Tetracene_B3LYP"  # Change this based on the geometry used (e.g., "H2O", "CO2", "Benzene", etc.)
