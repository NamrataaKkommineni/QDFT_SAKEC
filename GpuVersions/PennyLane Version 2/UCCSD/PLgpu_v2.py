    ...
    class PennyLaneVQESolver:
        def __init__(self, mapper, active_electrons, active_orbitals, initial_point=None, callback=None):
            ...
            if initial_point is not None and len(initial_point) == total_params:
                ...
                # 🔹 THE FIX: Mathematically identical random start
                np.random.seed(42)
                sigma = 0.001
                random_params = sigma * np.random.randn(total_params)
                self.initial_point = pnp.array(random_params, requires_grad=True)
