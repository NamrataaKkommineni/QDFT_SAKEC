...
def _main():
    ...
    class PennyLaneVQESolver:
        def __init__(self, mapper, active_electrons, active_orbitals, initial_point=None, callback=None):
            ...
            # 1. Generate Raw Excitations (Alternating Ordering)
            raw_singles, raw_doubles = qchem.excitations(self.electrons, self.num_qubits)

            # 2. Map Alternating wires to Qiskit's Block wires
            num_spatial = active_orbitals
            def map_wire(w):
                return w // 2 if w % 2 == 0 else (w - 1) // 2 + num_spatial

            # 3. Assign the mapped target wires directly (NO Z-STRING FORMATTER)
            self.singles = [[map_wire(w) for w in exc] for exc in raw_singles]
            self.doubles = [[map_wire(w) for w in exc] for exc in raw_doubles]
        ...
        def solve(self, problem):
            print("\n>> [PennyLane] Initiating Native CPU Pipeline...")
            ...
            # ---------------------------------------------------------
            # HARDWARE SAFE HAMILTONIAN BRIDGE (Bypasses pennylane-qiskit)
            # ---------------------------------------------------------
            pauli_words = []
            coeffs = []
            for term, coeff in qubit_op.label_iter():
                pauli_words.append(term)
                coeffs.append(coeff.real)

            pl_hamiltonian = qml.pauli.string_to_pauli_word(pauli_words[0]) * coeffs[0]
            for word, coeff in zip(pauli_words[1:], coeffs[1:]):
                pl_hamiltonian += qml.pauli.string_to_pauli_word(word) * coeff
            print(f"  [*] Bridged {len(qubit_op)} Pauli terms on {self.num_qubits} qubits.")

            # ---------------------------------------------------------
            # 2. The Native GPU/CPU Engine
            # ---------------------------------------------------------
            dev = qml.device("lightning.qubit", wires=self.num_qubits)
            
            # 🔹 THE FIX: Build Block-Ordered HF State to match Qiskit Hamiltonian
            num_spatial = self.num_qubits // 2
            num_alpha = self.electrons // 2
            num_beta = self.electrons // 2
            
            hf_array = np.zeros(self.num_qubits, dtype=int)
            hf_array[0 : num_alpha] = 1
            hf_array[num_spatial : num_spatial + num_beta] = 1
            hf_state = pnp.array(hf_array)

            # Because this is 100% native PennyLane, it compiles directly to the CPU/GPU
            @qml.qnode(dev, diff_method="adjoint")
            def cost_fn(params):
                # 1. Initialize the exact Block-ordered Hartree-Fock State
                qml.BasisState(hf_state, wires=range(self.num_qubits))
                
                # 2. Apply q-UCCSD Single Excitations (No Z-Strings)
                for i, s_wire in enumerate(self.singles):
                    qml.SingleExcitation(params[i], wires=s_wire)
                    
                # 3. Apply q-UCCSD Double Excitations (No Z-Strings)
                offset = len(self.singles)
                for j, d_wire in enumerate(self.doubles):
                    qml.DoubleExcitation(params[offset + j], wires=d_wire)
