...
def _main():
    ...
    # 1. Generate the standard Fermionic UCCSD
    std_ansatz = UCCSD(
        num_spatial_orbitals=active_num_spatial_orbitals,
        num_particles=(num_alpha, num_beta),
        qubit_mapper=mapper,
    ...
    # 2. Strip the Z-strings to create Qubit-UCCSD (q-UCCSD)
    q_uccsd_ops = []
    for op in std_ansatz.operators:
        # Replace 'Z' with 'I' to eliminate Jordan-Wigner strings
        new_paulis = [str(pauli).replace("Z", "I") for pauli in op.paulis]
        q_uccsd_ops.append(SparsePauliOp(new_paulis, op.coeffs))
    # 3. Build the highly efficient q-UCCSD circuit
    ansatz = EvolvedOperatorAnsatz(
        operators=q_uccsd_ops,
        initial_state=initial_state
    )
