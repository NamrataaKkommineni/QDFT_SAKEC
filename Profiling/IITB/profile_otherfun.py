...
class DFTEmbeddingSolver:
    ...
    def solve(self, driver: PySCFDriver,vqe_iter_log: list = None):
        _tic("DFT_reference")
        driver.run_pyscf()
        _toc("DFT_reference")
...
def _main():
    ...
    # --- IITB Ansatz Integration ---
    from qiskit.primitives import Estimator as StdEstimator
    
    # Generate an initial qubit_op to evaluate pruning
    base_problem = driver.run()
    as_problem_init = active_space.transform(base_problem)
    qubit_op_init = mapper.map(as_problem_init.hamiltonian.second_q_op())

    std_estimator = StdEstimator()
    optimal_list = []
    ...
    excitation_list_pauli_d = [mapper.map(ex) for ex in var_form_d.excitation_ops()]
    
    pruned_excitation_list_pauli = []
    
    for i, pauli_op in enumerate(excitation_list_pauli_d):
        var_form1 = EvolvedOperatorAnsatz(pauli_op, initial_state=initial_state)
        initial_job = std_estimator.run([var_form1], [qubit_op_init], parameter_values=[[0.0]])
        initial_energy = initial_job.result().values[0]
        
        vqe1 = VQE(std_estimator, var_form1, optimizer=optimizer, initial_point=[0.0])
        vqe_result = vqe1.compute_minimum_eigenvalue(qubit_op_init)
        E1 = np.real(vqe_result.eigenvalue)
        op_pt = vqe_result.optimal_point
        
        if abs(initial_energy - E1) > 1e-5:
            optimal_list.append(op_pt[0])
            pruned_excitation_list_pauli.append(pauli_op)
    ...
    initial_point = optimal_list + [0.0] * len(fer_excitation_op_s)
    # ---------------------------------
    ...
    try:
        result, embedding_energies = dft_solver.solve(driver,vqe_iter_log=vqe_iter_log)
