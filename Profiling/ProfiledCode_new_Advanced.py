...
# ================================================================
#  PROFILING GLOBAL STATE (MOVED TO TOP)P
# ================================================================
...
class DFTEmbeddingSolver:
    ...
    def solve(self, driver: PySCFDriver, omega: float):
        ...
        ### >>> ADDED: INTEGRAL TRANSFORMATION TIMING
        ...
        ### >>> ADDED: END INTEGRAL TRANSFORMATION TIMING
        ...
        while n_iter < self.max_iter:
            ...
            ### >>> ADDED: Active Space Transform Timing
            ...
            global _vqe_last_time         # <<< ADD THIS
            _vqe_last_time["t"] = None

            _tic("Solver_total")
            result = self.solver.solve(as_problem)
            
            # >>> CRITICAL FIX: Final timestamp update
            # This captures the time between the very last callback and the solver finishing
            if _vqe_last_time["t"] is not None:
                PROFILE["Quantum_VQE"] += (time.perf_counter() - _vqe_last_time["t"])
            else:
                print("Error in Calculation of Quantum VQE, VQE callback failed.")
...
# ---------- 🔹 CHANGE 3: Use AerEstimator (compatible with your setup) ----------
    ...
    def vqe_callback(eval_count, parameters, mean, std):
        ...
        global _vqe_last_time
        
        # 1. Capture the exact moment the solver hands control to the callback
        callback_start = time.perf_counter()
        
        # 2. Add the pure solver time (time elapsed since the LAST callback ended)
        if _vqe_last_time["t"] is not None:
            PROFILE["Quantum_VQE"] += (callback_start - _vqe_last_time["t"])

        # 3. --- CLASSICAL OVERHEAD HAPPENS HERE ---
        vqe_energy_history.append(mean)
        print(f"  [VQE] Eval {eval_count:3d}  Energy = {mean:.8f} Ha")
        # ------------------------------------------

        # 4. Capture the exact moment we hand control BACK to the solver
   ...
   # ============================================================
    ...
    print(f"Solver total (Pre/Post-processing time + Quantum VQE Time) : {PROFILE['Solver_total']:.2f} s")
    
    classical_qiskit_overhead = PROFILE['Solver_total'] - PROFILE['Quantum_VQE']
    ...
    print(f"Quantum VQE (callback only)  : {PROFILE['Quantum_VQE']:.2f} s")
