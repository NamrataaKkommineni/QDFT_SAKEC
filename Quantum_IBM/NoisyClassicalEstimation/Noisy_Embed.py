...
class DFTEmbeddingSolver:
    ...
    def solve(self, driver: PySCFDriver, omega: float):
        ...
        while n_iter < self.max_iter:
            ...
            mapper = TaperedQubitMapper(ParityMapper(num_particles = as_problem.num_particles))
            ...
            vqe_time = time.perf_counter() - start_time
            
            print(f"Time taken by VQE : {vqe_time:.2f}s")
...
def _main():
    ...
    class SPSATerminator:
        def __init__(self, threshold=1e-6):
            self.prev_energy = None
            self.threshold = threshold
            self.last_nfev = -1  # Tracks function evaluations to detect a new VQE run

        def __call__(self, nfev, parameters, value, stepsize, accepted):
            # If nfev drops, a brand new VQE optimization loop has started
            if nfev < self.last_nfev:
                self.prev_energy = None
            
            self.last_nfev = nfev

            if self.prev_energy is not None:
                diff = abs(value - self.prev_energy)
                if diff < self.threshold:
                    print(f"\n*** VQE CONVERGENCE REACHED VIA SPSA CHECKER ***")
                    print(f"Stopping SPSA optimization early (Diff: {diff:.8f} < {self.threshold}, Eval: {nfev}).")
                    return True  
            self.prev_energy = value
            return False

    # 2. Update your optimizer to include the terminator
    from qiskit_algorithms.optimizers import SPSA

    optimizer = SPSA(
        maxiter=5000,
        termination_checker=SPSATerminator(threshold=1e-3)
    )
...
# Build Hartree-Fock initial state (HF) and UCCSD ansatz
    ...
    # print(f"Circuit Depth is: {depth}")
    #initial_point = 0.05 * (2 * np.random.rand(ansatz.num_parameters) - 1)
...
# ---------- 🔹 CHANGE 3: Use AerEstimator (compatible with your setup) ----------
    ...
    probabilities = [[0.985, 0.015], [0.015, 0.985]]
    readout_error = ReadoutError(probabilities)
    noise_model.add_all_qubit_readout_error(readout_error)

    noisy_estimator = AerEstimator(
        backend_options={"noise_model": noise_model},
        run_options={"shots": 20000}
    )
    
    # ---------- 🔹 VQE callback (original normal behavior) ----------
    class VQECallback:
        def __init__(self, evals_per_iter=2, threshold=1e-3):
        ...
        def __call__(self, eval_count, parameters, mean, std):
            # AUTOMATIC RESET: Detects when the embedding loop triggers a fresh VQE run
            if eval_count == 1:
                self.current_iter = 0
                self.eval_in_iter = 0
                self.prev_energy = None
            ...
            if self.best_energy is None or mean < self.best_energy:
                self.best_energy = mean 
                
            self.energy_history.append(mean)
            self.eval_counts.append(eval_count)
            
            iter_num = ((eval_count - 1) // self.evals_per_iter) + 1
            
            if iter_num > self.current_iter:
            ...
            if self.prev_energy is not None:
                energy_diff = mean - self.prev_energy
                diff_str = f" | Diff: {energy_diff:+.8f} Ha"
                
                if abs(energy_diff) < self.threshold:
                    diff_str += f"  <-- [Threshold Met: < {self.threshold}]"
                    
            self.prev_energy = mean
            print(f"  Evaluation {self.eval_in_iter} (Total Eval: {eval_count:03d}) | Energy: {mean:.8f} Ha{diff_str}")

    vqe_callback = VQECallback(evals_per_iter=2, threshold=1e-3)
