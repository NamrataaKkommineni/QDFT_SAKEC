...
class DFTEmbeddingSolver:
    ...
    def solve(self, driver: PySCFDriver,mol_name: str = "Molecule", active_space_str: str = "CAS"):
        ...
        while n_iter < self.max_iter:
            ...
            # 3. Build the Penalized Hamiltonian
            # Benzene: 0.33828
            # Naphthalene: 0.21432
            # Anthracene : 0.14648
            # Tetracene  : 0.10110
            ...
            # Perylene : 0.12863
            beta = 0.12863  # <--- Ensure this matches the molecule being run!
...
def _main():
   ...
   "H  9.213265  -1.651184  0.000000"
    ...
    #beta = 0.46
    #Tuned
    mu = 0.33   # Range separation (ω)
    alpha = 0.19  # Short-range HF fraction
    beta = 0.09  # Long-range addition (Total LR = alpha + beta = 1.0)
...
# Build Hartree-Fock initial state (HF) and UCCSD ansatz
    ...
    # 1. Run your custom PySCF calculator manually to bypass the LibXC reset bug
    driver._calc.kernel()

    # 2. Safely generate the full Qiskit problem using the optimized MOs
    from qiskit_nature.second_q.problems import ElectronicBasis
    full_problem = driver.to_problem(basis=ElectronicBasis.MO, include_dipole=False)

    # 3. Prepare the active space transformer with full system sizes
    active_space.prepare_active_space(
        full_problem.num_particles,
        full_problem.num_spatial_orbitals,
        occupation_alpha=full_problem.orbital_occupations,
        occupation_beta=full_problem.orbital_occupations_b,
    )

    # 4. Truncate down to your active space (e.g., 6e, 6o)
    as_problem = active_space.transform(full_problem)

    # 🔹 BYPASS QISKIT NATURE BUG (Issue #645)
    # Replace the populated beta block with an empty PolynomialTensor
    # so MP2InitialPoint registers the RKS setup correctly.
    if not as_problem.hamiltonian.electronic_integrals.beta.is_empty():
        as_problem.hamiltonian.electronic_integrals.beta = PolynomialTensor({})

    # 5. Safely compute the MP2 initial amplitudes using the exact active space problem!
    mp2_init = MP2InitialPoint()
    mp2_init.compute(ansatz=ansatz, problem=as_problem)
    initial_point = mp2_init.to_numpy_array()

    print("\n[Success] MP2 Initial Point computed safely!")
    # #initial_point = 0.05 * (2 * np.random.rand(ansatz.num_parameters) - 1)
...
# ---------- 🔹 VQE callback with quantum timing & convergence tracking ----------
    ...
    # ================================================================
    try:
        mol_name = "Perylene_tuned"  # Change this based on the geometry used (e.g., "H2O", "CO2", "Benzene", etc.)
