...
class DFTEmbeddingSolver:
    ...
    def solve(self, driver: PySCFDriver, omega: float):
        ...
        try:
            ...
            # Adjusted to safely check for 'use_range_separation' in case it wasn't defined in _init_
            ...
            eigvals = np.linalg.eigvalsh(fock_total)
        ...
        try:
            ...
            cubegen.orbital(driver._calc.mol, "homo.cube", driver._calc.mo_coeff[:, nocc-1])
            cubegen.orbital(driver._calc.mol, "lumo.cube", driver._calc.mo_coeff[:, nocc])
            print("[Saved] homo.cube, lumo.cube")
        except Exception as e:
